...
import { Delete, MoreVert, Restore } from "@mui/icons-material";
...
function Existing() {
    ...
    useEffect(() => {
        const dataView1 = datalist.map(item =>
            <Card variant="outlined" key={item.id} onClick={(eve)=>{
                setItem(item)
                // handleCardClick()
            } }sx={{ mb: 2, width: "90%", padding: "0px", margin: "5px" }} className="card">
                <CardHeader
    ...
    return (
        <div className="container-fluid">
            ...
            <BasicModal item={Item} openModal={openModal} />
            <CreateModal/>
        </div>
    );
}
